...
def clip_streamline_to_fov(stream, new_shape, use_gpu=True, epsilon=1e-6):
    ...
    epsilon : float, optional
        Small tolerance for boundary checks, by default 1e-6.
    ...
    if use_gpu:
        try:
            import cupy as xp
            print("Using GPU acceleration for streamline clipping")
        except ImportError:
            print("Could not import cupy, falling back to CPU for streamline clipping")
            import numpy as xp
            use_gpu = False
    else:
        import numpy as xp
    ...
    if use_gpu:
        try:
            ...
            print(f"Error converting segments to numpy: {e}")
            return segments
    else:
        import numpy as np
...
def interpolate_to_fov(p1, p2, new_shape, use_gpu=True):
    """
    Interpolating a point on the boundary of the field of view.
...
def transform_streamline(s_mm, A_new_inv, use_gpu=True):
    """
    Transforming a streamline from mm space to voxel space.
    ...
    if use_gpu:
        try:
            ...
            print("Could not import GPU libraries, falling back to CPU for streamline transformation")
            import numpy as xp
            use_gpu = False
...
def transform_and_densify_streamlines(
    ...
    """
    Transforming streamlines from mm space to voxel space and applying densification.
    ...
    for s in transformed_streams:
        ...
        if np.any(mask):
            filtered_points = [s[i] for i, in_volume in enumerate(mask) if in_volume]
    ...
    print(
        f"Clipping: {clipped_count}/{total_streamlines} streamlines retained ({clipped_count / total_streamlines * 100:.1f}%)")
    if clipped_count < total_streamlines * 0.5:
        print("Over 50% of streamlines were clipped!")
    if clipped_count == 0:
        print("All streamlines were clipped!")
        return []
