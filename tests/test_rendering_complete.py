"""
Comprehensive tests for the rendering module.
Tests all functions in rendering/ folder including:
...
# Add rendering module to path
import sys
project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, project_root)

# Try to import rendering modules - handle import errors gracefully
try:
...
def test_module_structure():
    """Test that rendering module structure is correct"""
    rendering_path = os.path.join(project_root, 'rendering')
    
    # Check if rendering directory exists
    assert os.path.exists(rendering_path)
    assert os.path.isdir(rendering_path)
    ...
    for filename in expected_files:
        filepath = os.path.join(rendering_path, filename)
