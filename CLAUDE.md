...
pytest tests/test_nifti_resampling.py  # narrow run while editing
python run_comprehensive_tests.py         # custom integration runner
```

`tests/conftest.py` adds `preprocessing/` to `sys.path` so tests can import either via the package or as standalone scripts (see "Import patterns" below). Many tests need real NIfTI/TRK/OME-Zarr data and are expensive — prefer the narrowest useful test and state what was skipped.
...
Batch over a directory of TRK files sharing one NIfTI: `python batch_processing.py --nifti brain.nii.gz --trk-dir ./trk_files/`.

3D synthetic training entry point: `python training/train_3d.py --on_the_fly --trk_dir ... --input_nifti ... --checkpoint_dir ... --no_wandb`.

```bash
# Sanity-check a trained checkpoint against a known synthetic patch (dice ≈ 0.98 → inference path OK)
python training/sanity_check_synthetic.py --checkpoint best_3d.ckpt --voxel_size 0.001
# Sanity-check the thin-slab sliding-window path
python training/sanity_check_thinslab.py --checkpoint best_3d.ckpt
# Test inference on a specific OME-Zarr region
python predict_real_region.py --zarr /path/to/volume.ome.zarr --checkpoint best_3d.ckpt --normalize percentile
```
...
- [preprocessing/](preprocessing/) — core MRI/tractography processing. [full_volume.py](preprocessing/full_volume.py) (`process_and_save`) is the traditional full-volume path; [patch_extraction.py](preprocessing/patch_extraction.py) is the default patch-first path; [nifti_resampling.py](preprocessing/nifti_resampling.py), [streamline_processing.py](preprocessing/streamline_processing.py), [densify.py](preprocessing/densify.py), [ants_transform.py](preprocessing/ants_transform.py), [gpu_utils.py](preprocessing/gpu_utils.py).
- [rendering/](rendering/) — rendering. [example_generation.py](rendering/example_generation.py) builds 2D synthetic examples; [slice_renderer.py](rendering/slice_renderer.py) saves NIfTI/TRK visualizations and masks; [volume_renderer.py](rendering/volume_renderer.py) does 3D rendering with streamlines; [slice_augmentation.py](rendering/slice_augmentation.py) drives the weighted preset selection (clean 30% / subtle 30% / moderate 20% / heavy 20%); [volume_artifact_augmentation.py](rendering/volume_artifact_augmentation.py) is image-only 3D realism for training.
- [training/](training/) — PyTorch Lightning 3D training, OME-Zarr loading, prediction, preview. Notable: [unet3d.py](training/unet3d.py), [losses.py](training/losses.py), [datamodules/datasets.py](training/datamodules/datasets.py) (`SyntheticDataset3D` / `OnTheFlySyntheticData3D`), [datamodules/dataloaders.py](training/datamodules/dataloaders.py) (Lightning DataModules), [datamodules/omezarr.py](training/datamodules/omezarr.py) (physical-scale patch extraction → fixed output tensor shapes), [precompute_patches_3d.py](training/precompute_patches_3d.py).
...
[batch_processing.py](batch_processing.py) wraps `process_batch` (CLI + Python API) and `process_patches_inmemory` (no file I/O — returns `images, masks` arrays). It auto-tunes patch counts per file: >100k streamlines → more patches, <10 → minimal patches.
...
- **CPU fallback is mandatory.** All GPU code goes through [preprocessing/gpu_utils.py](preprocessing/gpu_utils.py) (`try_gpu_import` → `xp = numpy or cupy`). Don't hard-import `cupy`, CUDA, or MPS.
...
- **Coordinate systems are easy to break.** Streamlines, RAS, voxel coords, NIfTI affines, and ANTs transforms (warp + inverse warp + affine, converted to RAS+ for TrackVis) all interact in `preprocessing/`. Be careful around `ants_transform.py` and `streamline_processing.py`.
...
Generating training data to match high-resolution light-sheet (SPIM/OME-Zarr) inference at ~1µm. At these scales the patch FOV (`patch_size * voxel_size`) is far smaller than the source NIfTI voxels and the native streamline spacing, which exposed several issues — all now handled in [preprocessing/patch_extraction.py](preprocessing/patch_extraction.py) and [rendering/volume_renderer.py](rendering/volume_renderer.py):
...
- **Train/inference normalization MUST match.** OME-Zarr inference ([datamodules/omezarr.py](training/datamodules/omezarr.py)) uses 1–99 percentile → [0,1]. Both synthetic 3D paths in [datamodules/datasets.py](training/datamodules/datasets.py) now do the same. Never reintroduce min-max normalization (outlier-sensitive — one bright fiber sets the scale).
...
```bash
sbatch training/precompute_patches.sh   # writes training/precomputed_patches/<trk_stem>/*_3d.nii.gz
# CONFIRM a healthy count before training:
find training/precomputed_patches -name '*_3d.nii.gz' | wc -l
sbatch training/train_cached.sh         # --cached_patches --num_workers 12, fresh checkpoint dir
```
...
- **Real-LSM transfer proxy (unlabeled).** `RealLSMProxyCallback` in `train_3d.py` runs the model each val epoch on a fixed 3×3 OME-Zarr center grid and logs `real_pred_pos_frac` (mean) and `real_pred_pos_frac_median` (watch this — robust to one hot region). Enable with `--real_proxy_zarr <path>` (default `"None"` → no-op). **Calibrated baseline (2026-06-04):** regions 1–3 median = 0.000200; epoch-149 training run baseline = 0.00140. Anchor all step-(b) movement against the *same run's own* epoch-0 baseline, not the calibration figure. `calibrate_real_proxy.py` re-derives it on cluster GPU (OOMs on laptop).

## 3D training stability and inference

- **Use `bf16-mixed`, not `16-mixed`.** fp16 overflows (~65504) in the forward pass — the AMP GradScaler only guards gradients, so a forward-pass overflow NaN-poisons the weights permanently (seen as: dice climbs to ~0.35 then collapses to flat-NaN around step ~10k). bf16 has fp32 range, needs no scaler, free on H100/H200. `train_3d.py` now auto-selects bf16 when supported and sets `gradient_clip_val=1.0`. A NaN-poisoned `last.ckpt` will reload on resume — start fresh (`--no_resume` / new `--checkpoint_dir`).
...
| Priority | File | Bug |
|---|---|---|
| — | — | No open known bugs as of 2026-09-18. |

Fixed during the 2026-09 cleanup (do not reintroduce): `process_and_save()` now always returns a dict (`synthesis_outputs`, `error`); the phantom `--force_compression` block and duplicate `__main__` in `preprocessing/full_volume.py` are gone; the `[STREAMLINE DEBUG]` / `DEBUG VIOLATIONS` prints are removed or gated behind `debug`; the CPU full-volume resampler now uses `scipy.ndimage.map_coordinates(order=3)` (cubic, matching the GPU path); `soft_cldice`/`soft_dice_cldice` honour `iter_` (default 10, the value all runs used); hermite densification re-samples by arc length so consecutive points respect `step_size`.
