...
Usage:
    python preprocessing/full_volume.py --input brain.nii.gz --trk fibers.trk [options]
"""
...
def process_and_save(
    ...
    """Resample a NIfTI + TRK pair to a new grid and save ``{output_prefix}.nii`` / ``.trk``.

    Returns a dict with keys ``synthesis_outputs`` (``{'nifti': path, 'trk': path}``
    on success, ``None`` on failure) and ``error`` (``None`` on success).
    """

    for label, path in (("NIfTI", original_nifti_path), ("TRK", original_trk_path)):
        if not os.path.isfile(path):
            raise FileNotFoundError(f"Input {label} file not found: {path}")
    ...
    if hasattr(new_data, 'get'):
        new_data_np = new_data.get()
    else:
    ...
    # Always write an uncompressed .nii: downstream steps (syntract.py) expect
    # ``{output_prefix}.nii`` and gzip on multi-GB volumes dominates run time.
    ...
    if len(densified_vox) == 0:
        ...
        return {'synthesis_outputs': None, 'error': 'No streamlines survived processing'}
    ...
    if use_ants:
        # Convert A_new to NumPy if it's a CuPy array
        A_new_np = A_new.get() if hasattr(A_new, 'get') else A_new
        new_trk_header = {
            "dimensions": np.array(target_dimensions, dtype=np.int16),
            "voxel_sizes": np.sqrt(np.sum(A_new_np[:3, :3] ** 2, axis=0)).astype(np.float32),
            "voxel_to_rasmm": A_new_np.astype(np.float32)
        }
    else:
        trk_obj = nib.streamlines.load(original_trk_path)
        ...
        # Convert A_new to NumPy if it's a CuPy array
        A_new_np = A_new.get() if hasattr(A_new, 'get') else A_new
        new_voxsize = np.sqrt(np.sum(A_new_np[:3, :3] ** 2, axis=0))
        new_trk_header["voxel_sizes"] = new_voxsize.astype(np.float32)
        new_trk_header["voxel_to_rasmm"] = A_new_np.astype(np.float32)
    ...
    return {
        'synthesis_outputs': {'nifti': out_nifti_path, 'trk': out_trk_path},
        'error': None,
    }
