...
def rbf_interpolate_streamline(streamline, step_size, function='thin_plate', epsilon=None, xp=np, preserve_length=True):
    ...
    # Create parameterization (batch_processing distance)
    ...
    if preserve_length and len(result) > 2:
        ...
        if abs(length_ratio - 1.0) > 0.001:  # More than 0.1% difference
            ...
            # Calculate batch_processing distances for the RBF result
...
def _resample_by_arc_length(points: np.ndarray, step: float) -> np.ndarray:
    """Re-space a polyline so consecutive points are at most ``step`` apart.

    Cubic Hermite evaluation on a uniform *parameter* grid is not uniform in
    *arc length* (the tangent magnitudes change the parametric speed), so the
    interpolated curve is re-sampled here along its own length. The geometry of
    the curve is unchanged; only where the samples sit on it.
    """
    ...
    total = float(cum[-1])
    if total <= 0.0 or float(seg.max()) <= step:
        return points
    n_out = int(np.ceil(total / step)) + 1
    targets = np.linspace(0.0, total, n_out)
    out = np.empty((n_out, points.shape[1]), dtype=np.float32)
    for dim in range(points.shape[1]):
        out[:, dim] = np.interp(targets, cum, points[:, dim])
    return out
...
def densify_streamline_subvoxel(streamline, step_size, use_gpu=True, interp_method='hermite', voxel_size=1.0):
    ...
    try:
        ...
        if interp_method == 'hermite':
            densified_streamline = _resample_by_arc_length(densified_streamline, float(adaptive_step))
