...
from batch_processing import process_patches_inmemory  # type: ignore
...
def render_trk(
    ...
    # uncompressed .nii (use_compressed_nifti=not skip_2d_viz in batch_processing.py),
...
def main():
    ...
    if not os.path.isdir(args.trk_dir):
        raise FileNotFoundError(f"--trk_dir does not exist or is not a directory: {args.trk_dir}")
    if not os.path.isfile(args.input_nifti):
        raise FileNotFoundError(f"--input_nifti not found: {args.input_nifti}")
